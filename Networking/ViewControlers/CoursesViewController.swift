...
class CoursesViewController: UIViewController {
    ...
    //Метод грузить через NetworkManager
    func fetchData() {
        
        NetworkManager.fetchData(url: url) { courses in
            //Масив який приняли передамо в свойство courses, та обновимо tableView
    ...
    //Метод відправляє данні на сервер
    func postRequest() {
        AlamofireNetworkRequest.postRequest(url: postRequestUrl) { courses in
            //Передаємо масив з курсами в масив моделі, для того щоб відобразити це на екрані
            self.courses = courses
            DispatchQueue.main.async {
                self.tableView.reloadData()
            }
